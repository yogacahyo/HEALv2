...
const statusColors: Record<string, string> = {
  ...
  'Belum Absen': 'bg-surface-container-high text-on-surface-variant border-outline-variant',
...
export function StatusBadge({ status, size = 'sm' }: StatusBadgeProps) {
  const colorClass = statusColors[status] || 'bg-surface-container-high text-on-surface-variant border-outline-variant';
  ...
  return (
    <span className={`inline-flex items-center rounded-full border font-semibold ${colorClass} ${sizeClass}`}
      style={{ boxShadow: '1px 1px 3px rgba(0,0,0,0.04), inset 1px 1px 2px rgba(255,255,255,0.5)' }}>
      {status}
    </span>
  );
}
